import {S, esc, getActiveLiga, pById, pFN, calcGlobal, calcPtsJornada, toast} from './state.js';
...
export async function exportGruposWhatsApp(){
  ...
  const grid=turnos.map(turno=>{
    ...
    for(let c=1;c<=canchasN;c++){
      ...
      // Nombres desde los partidos ya generados (a1/a2/b1/b2), no desde
      // p.grupo actual — si hubo una promoción después de generar esta
      // jornada, p.grupo ya no refleja quién jugó aquí realmente.
      const pids=[...new Set(ms.filter(m=>m.grupo===g).flatMap(m=>[m.a1,m.a2,m.b1,m.b2]))];
      const names=pids.map(pById).filter(Boolean).sort((a,b)=>(a.orden||0)-(b.orden||0)).slice(0,4).map(p=>p.nombre);
