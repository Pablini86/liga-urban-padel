...
// La fila tiene 4 <span> (⠿, número, nombre, G-label) — el índice del label de
// grupo es 3, no 4. Con el índice equivocado esto tronaba en el primer
// renglón y dejaba de repintar los números/G-labels durante el arrastre (el
// orden que se guarda al final sigue siendo correcto porque saveOrder lee el
// orden real del DOM, no estas etiquetas).
function updDrag(){document.querySelectorAll('#dl .di').forEach((item,i)=>{item.querySelectorAll('span')[1].textContent=i+1;item.querySelectorAll('span')[3].textContent='G'+(Math.floor(i/4)+1);});}
